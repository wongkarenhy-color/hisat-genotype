...
# --------------------------------------------------------------------------- #
# Function to flatten an allele tree into an array                            #
# --------------------------------------------------------------------------- #
def flatten(tree,
            prev_key = '', 
            sep      = '*',
    ...
    for key, value in tree.items():
        ...
        if value['children'] is None:
            ...
            if trim > cur_lvl:
                items.extend(flatten(value['children'], 
                                     new_key, 
                                     ':',
                                     trim    = trim,
                                     cur_lvl = cur_lvl + 1).items())
            if trim == 4 or trim == cur_lvl:
                items.append((new_key + ' - Trimmed', value['score']))
        ...
        for type_ in report_results[file_]:
            ...
            if type_ == 'Allele splitting':
                ...
                for gene in tree:
                    ...
                    flattened_tree = flatten(tree[gene]['children'], 
                                             gene, 
                                             trim = args.trim_level)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--trim",
                        ...
                        type    = int,
                        default = 4,
                        help    = "Trim allele to specific field level "\
                                    "(example : A*01:01:01:01 trim 2 A*01:01)")
